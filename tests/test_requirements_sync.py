...
def test_requirements_lock_covers_every_direct_pin():
    ...
    lock = {_normalize_pin(line) for line in _read_lines(ROOT / "requirements-lock.txt")}
    assert lock, "requirements-lock.txt must pin at least one dependency"
    missing = [dep for dep in direct if _normalize_pin(dep) not in lock]
    assert (
        missing == []
    ), f"requirements.txt dependencies missing from requirements-lock.txt: {missing}"


def test_requirements_lock_is_sorted_and_pinned():
    lines = _read_lines(ROOT / "requirements-lock.txt")
